...
# only use 25% of available dataset (~ 12.5k structure, x6)
...
print(f"train size: {len(train)}, test: {len(test)}, val: {len(val)}")
...
try:
    for lr in lrs:
        ...
        save_model_path = MODELS_PATH + "EM250x300CNNTunedDecayingLR{}_By{}_every4".format(lr, gamma)
        torch.save(model.state_dict(), save_model_path)
        print("EM250x300CNN forward model trained and saved for lr={}, decaying by {} every 4 epochs, for {} epochs".format(lr, gamma, epochs))
...
LOSS_DIR = "/scratch/gpfs/sbfisher/EM250x300/results/"
